    ...
    for org in orgDict.keys():
        ...
        for uplink in uplinks:
            if uplink.serial in devMap:
                ...
                devList[i].wan1PubIp = ' '
                if uplink.wan1_ip != uplink.wan1_pub_ip:
                    devList[i].wan1PubIp = uplink.wan1_pub_ip
                devList[i].wan1Gateway = uplink.wan1_gateway
                devList[i].wan2Ip = uplink.wan2_ip
                devList[i].wan2PubIp = ' '
                if uplink.wan2_ip != uplink.wan2_pub_ip:
                    devList[i].wan2PubIp = uplink.wan2_pub_ip
                devList[i].wan2Gateway = uplink.wan2_gateway
                if devList[i].wan1Stat == 'active' or devList[i].wan1Stat == 'ready':
                    devList[i].wan1Color = 'green'
                elif  devList[i].wan1Stat == 'failed' or devList[i].wan1Stat == 'not connected':
                    devList[i].wan1Color = 'red'
...
def toCSV(errorList, devList):
    ...
    if devList != []:
        with open('/home/ccorby/meraki/project1_ccorby/meraki_lib/reports/uplink_log.csv', 'w', newline='\n') as f:
            ...
            writer.writerow(['Device Name', 'Network Name', 'Last Reported At', 'WAN1 Status', 'WAN1 Actual IP', 'WAN1 Displayed IP', 'WAN1 Gateway',  'WAN2 Status', 'WAN2 Actual IP', 'WAN2 Displayed IP', 'WAN2 Gateway', 'Last Failover At'])
            for dev in devList:
                ...
                writer.writerow([dev.name, dev.net, dev.lastReported, dev.wan1Stat, dev.wan1Ip, dev.wan1PubIp, dev.wan1Gateway, dev.wan2Stat, dev.wan2Ip, dev.wan2PubIp, dev.wan2Gateway, dev.lastFailover])
...
def toEmail(errorList, devList):
    """
    Parses the list of objects to create the html table, csv, and send the email through outlook.
    
    :usage: toEmail(list of potential errors, list of device objects) -> outlook email
    :param devList: List of MX device objects
    ...
    recipients = ['ccorby@baillie.com', 'engineering@baillie.com', 'servicedesk@baillie.com']
    ...
    if devList != []:
        for dev in devList:
            ...
            <td>{dev.wan1Ip}</td>
            <td>{dev.wan1PubIp}</td>
            <td>{dev.wan1Gateway}</td>
            <td bgcolor={dev.wan2Color}>{dev.wan2Stat}</td>
            <td>{dev.wan2Ip}</td>
            <td>{dev.wan2PubIp}</td>
            <td>{dev.wan2Gateway}</td>
            </tr>
            """
            tableRows += row
        time = datetime.datetime.now()
        ...
        <html>
          ...
          <body>
            <table>
              <tr><th colspan='10'>Status Report of Uplinks for Security Devices</th></tr>
              <tr>
                 <th>Device</th>
                 <th>Network</th>
                 <th>WAN1 Status</th>
                 <th>WAN1 Actual IP</th>
                 <th>WAN1 Displayed IP</th>
                 <th>WAN1 Gateway</th>
                 <th>WAN2 Status</th>
                 <th>WAN2 Actual IP</th>
                 <th>WAN2 Displayed IP</th>
                 <th>WAN2 Gateway</th>
              {tableRows}
              <tr>
                <td>Timestamp</td>
                <td colspan='9'>{time}</td>
              </tr>
            </table>
            <br>
            <table>
