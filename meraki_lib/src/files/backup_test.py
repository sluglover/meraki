...
def initBackup(API_KEY):
    ...
    retList = []
    with open('/home/ccorby/meraki/project1_ccorby/meraki_lib/reports/functions_post.csv', 'w', newline='\n') as f:
        writer = csv.writer(f, delimiter = ',')
        writer.writerow(['Function', 'Parameters', 'Match', 'Put/Post'])
        postList = []
        actualPosts = []
        postParams = []
        putList = []
        ...
        for path in spec['paths']:
            ...
            paramList = []
            if 'get' in functions and 'post' in functions and 'parameters' in functions['get']:
                for param in functions['get']['parameters']:
                    if 'required' in param:
                        paramList.append(param['name'])
                actualPosts.append(functions['get']['operationId'])
                if functions['get']['operationId'][-1] == 's' and functions['get']['operationId'][-2] == 'e' and ((functions['get']['operationId'][-3] == 'h' and functions['get']['operationId'][-4] == 'c') or (functions['get']['operationId'][-3] == 's' and functions['get']['operationId'][-4] == 's')):
                    postList.append(functions['get']['operationId'][:-2])
                elif functions['get']['operationId'][-1] == 's' and functions['get']['operationId'][-2] == 'e' and functions['get']['operationId'][-3] == 'i':
                    postList.append(functions['get']['operationId'][:-3] + 'y')
                elif functions['get']['operationId'][-1] == 's':
                    postList.append(functions['get']['operationId'][:-1])
                else:
                    postList.append(functions['get']['operationId'])
                postParams.append(paramList)
            elif 'get' in functions and 'put' in functions and 'parameters' in functions['get']:
                for param in functions['get']['parameters']:
                    if 'required' in param:
                        paramList.append(param['name'])
                putList.append(functions['get']['operationId'])
                putParams.append(paramList)
        acc = 0
        for post in postList:
            ...
            if post in putList:
                ...
                both = 'post'
            writer.writerow([actualPosts[acc], postParams[acc], match, both])
            acc += 1
        acc = 0
        match = 'N'
        both = 'put'
        for put in putList:
            if put not in postList:
                writer.writerow([put,putParams[acc], match, both])
                acc += 1
